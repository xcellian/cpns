...
$(document).ready(function() {
	...
	var searchItem = $('#selSearchItem').kendoMultiSelect({
		...
		autoClose     : false,
		dataSource    : [
			{itemLabel: '할인키코드', itemValue: 'cpnCd'},
			{itemLabel: '할인키명',   itemValue: 'cpnNm'},
			{itemLabel: '비고',       itemValue: 'remark'}
		],
		change        : function() {
	...
	var gridCpnList = $('#gridCpnList').DataTable({
		...
		columns       : [
			...
			{data: 'cpnCd',  name: 'cpnCd',  title: '할인키코드'},
			{data: 'cpnNm',  name: 'cpnNm',  title: '할인키명'},
			{data: 'price',  name: 'price',  title: '판매금액',   className: 'text-right',  render: $.fn.dataTable.render.number(',', '.', 0)},
			{data: 'remark', name: 'remark', title: '비고'},
		...
		columnDefs    : [
			...
			{
				render: function(data, type, row) {
					return data == '0'? '사용불가': '사용';
				},
				targets: 5
			},
			{
				render: function(data, type, row) {
					return '<span class="btnCpnEdit k-icon k-icon-14 k-icon-blue k-i-edit pointer" title="상세"></span>';
				},
				targets: 7
			}
		],
		ajax          : {
			...
			data    : function(data) {
				...
				if (<c:out value="${searchCond.initialized}" escapeXml="true" /> && pageReload) {
					//sort order 
					$('#gridCpnList').DataTable().order([<c:forEach var="item" items="${searchCond.order}">['${item.column}', '${item.dir}'],</c:forEach>]);
					
					//page length
					$('#gridCpnList').DataTable().page.len(<c:out value="${searchCond.length}" escapeXml="true" />);
				}
				else {
				...
				data.cpnType = '<c:out value="${cpnType}" escapeXml="true" />';
	...
	$('#gridCpnList').on('init.dt', function() {
		if (<c:out value="${searchCond.initialized}" escapeXml="true" /> && pageReload) {
			pageReload = false;
			var pageIdx = <c:out value="${searchCond.page}" escapeXml="true" />;
			if (pageIdx > 0) {
				gridCpnList.page(pageIdx).draw('page');
			}
		}
	}).DataTable();
	
	//페이징 이벤트
	$('#gridCpnList').on('page.dt', function() {
		searchCond.page = gridCpnList.page();
	}).DataTable();
	...
	$('#btn-search').click(function(e) {
		...
		fnSetSearchCond();
		gridCpnList.order(defaultOrder).draw();
	});
	
	//할인키 상세정보 화면 표시 action binding
	$('#gridCpnList tbody').on('click', 'tr .btnCpnEdit', function(e) {
		e.stopPropagation();
		
		var data = gridCpnList.row($(this).closest('tr')).data();
		location.href = '<c:url value="/cpns/cpns/"/>' + data.cpnSeq + '?' + $.param(gridCpnList.ajax.params());
	});
});
</script>
</body>

			...
			<div class="main-section k-content pd-3">
				<form id="instForm" data-role="validator" novalidate="novalidate">
					<ul class="fieldlist">
						...
						<li>
							<label for="input-cpnCd" class="required" style="display: inline-block;">쿠폰코드</label>
							<button id="btn-cpnCdDupChk" type="button" class="k-button slim chocolate float-r" style="position: relative; top: -5px;">
								<span class="k-icon k-icon-13 k-i-check" style="top: -1px;"></span> 쿠폰코드 중복확인
							</button>
							<input id="input-cpnCd" name="쿠폰코드" type="text" class="k-textbox" style="width: 100%;" value="" autocomplete="off" 
								data-maxLen="5" required placeholder="영숫자만 입력 가능합니다." />
						</li>
						<li>
							<label for="input-cpnNm" class="required">쿠폰명</label>
							<input id="input-cpnNm" name="쿠폰명" type="text" class="k-textbox" style="width: 100%;" value="" autocomplete="off" 
...
$(document).ready(function () {
	...
	var validator = $("#instForm").kendoValidator({
		...
		messages: {
			...
			cdFormat   : '쿠폰코드는 영숫자 및 특수문자( _ )만 입력 가능합니다.'
		}
	}).data("kendoValidator");
	...
	$('#btn-cpnCdDupChk').click(function(e) {
		...
		if (!(chkRetParkCd && chkRetCpnCd)) {
			fnAlertError($('#dialog'), {
				content: '주차장 및 쿠폰코드는 필수 입력 항목입니다.',
			...
			success : function(result, status, jqXHR) {
				if (result.retMsg == 'common.message.success') {
					...
					if (result.checkResult) {
						...
						fnAlertWarn($('#dialog'), {
							content: '동일한 쿠폰코드가 존재합니다.<br/>다른 쿠폰코드를 입력해 주세요.',
							actions: [
								{
									text   : 'OK',
									primary: true,

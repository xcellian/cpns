...
,__isGecko:function(){return (window.navigator.mozApps||window.navigator.buildID)&&window.navigator.product===k&&window.navigator.userAgent.indexOf(l)==-1&&window.navigator.userAgent.indexOf(n)==-1;}
...
;return Q;}
,__ids:{"Windows NT 10.0":J,"Windows NT 6.3":O,"Windows NT 6.2":L,"Windows NT 6.1":n,"Windows NT 6.0":K,"Windows NT 5.2":h,"Windows NT 5.1":bv,"Windows NT 5.0":N,"Windows 2000":N,"Windows NT 4.0":bx,"Win 9x 4.90":C,"Windows CE":m,"Windows 98":H,"Win98":H,"Windows 95":w,"Win95":w,"Mac OS X 10_12":s,"Mac OS X 10.12":s,"Mac OS X 10_11":o,"Mac OS X 10.11":o,"Mac OS X 10_10":bt,"Mac OS X 10.10":bt,"Mac OS X 10_9":t,"Mac OS X 10.9":t,"Mac OS X 10_8":y,"Mac OS X 10.8":y,"Mac OS X 10_7":g,"Mac OS X 10.7":g,"Mac OS X 10_6":T,"Mac OS X 10.6":T,"Mac OS X 10_5":q,"Mac OS X 10.5":q,"Mac OS X 10_4":W,"Mac OS X 10.4":W,"Mac OS X 10_3":d,"Mac OS X 10.3":d,"Mac OS X 10_2":D,"Mac OS X 10.2":D,"Mac OS X 10_1":b,"Mac OS X 10.1":b,"Mac OS X 10_0":V,"Mac OS X 10.0":V},getVersion:function(){var bG=qx.bom.client.OperatingSystem.__getVersionForDesktopOs(navigator.userAgent);if(bG==null){bG=qx.bom.client.OperatingSystem.__getVersionForMobileOs(navigator.userAgent);}
;if(bG!=null){return bG;}
else {return Q;}
;}
,__getVersionForDesktopOs:function(bH){var bK=[];for(var bJ in qx.bom.client.OperatingSystem.__ids){bK.push(bJ);}
;var bL=new RegExp(P+bK.join(u).replace(/\./g,z)+i,br);var bI=bL.exec(bH);if(bI&&bI[1]){return qx.bom.client.OperatingSystem.__ids[bI[1]];}
;return null;}
,__getVersionForMobileOs:function(bR){var bM=bR.indexOf(e)!=-1;var bS=bR.indexOf(F)!=-1;var bN=bR.match(/(iPad|iPhone|iPod)/i)?true:false;if(bM){var bU=new RegExp(/Windows Phone (\d+(?:\.\d+)+)/i);var bP=bU.exec(bR);if(bP&&bP[1]){return bP[1];}
;}
...
,__hiddenScrollbars:{visible:true,hidden:true},getContentWidth:function(s){var p=qx.bom.element.Style;var q=qx.bom.element.Style.get(s,j);var r=parseInt(p.get(s,f)||d,10);var v=parseInt(p.get(s,e)||d,10);if(this.__hiddenScrollbars[q]){var u=s.clientWidth;if((qx.core.Environment.get(b)==g)||qx.dom.Node.isBlockNode(s)){u=u-r-v;}
;if(qx.core.Environment.get(b)==a){if(u===0&&s.offsetHeight===0){return s.offsetWidth;}
;}
;return u;}
else {if(s.clientWidth>=s.scrollWidth){return Math.max(s.clientWidth,s.scrollWidth)-r-v;}
else {var t=s.scrollWidth-r;if(qx.core.Environment.get(b)==a){t-=v;}
;return t;}
;}
;}
,getContentHeight:function(A){var w=qx.bom.element.Style;var z=qx.bom.element.Style.get(A,k);var y=parseInt(w.get(A,i)||d,10);var x=parseInt(w.get(A,h)||d,10);if(this.__hiddenScrollbars[z]){return A.clientHeight-y-x;}
else {if(A.clientHeight>=A.scrollHeight){return Math.max(A.clientHeight,A.scrollHeight)-y-x;}
else {return A.scrollHeight-y;}
;}
;}
}});}
)();
...
,get:function(bd,be){if(bd.tagName==y){var location=this.__getBodyLocation(bd);var bh=location.left;var top=location.top;}
else {var W=this.__computeBody(bd);var bb=this.__computeOffset(bd);var scroll=this.__computeScroll(bd);var bh=bb.left+W.left-scroll.left;var top=bb.top+W.top-scroll.top;}
;var X;var bc;if(bd instanceof SVGElement){var bi=bd.getBoundingClientRect();X=bi.width;bc=bi.height;}
else {X=bd.offsetWidth;bc=bd.offsetHeight;}
;var Y=bh+X;var ba=top+bc;if(be){if(be==p||be==b){var bg=qx.bom.element.Style.get(bd,z);if(bg==b||bg==g){Y+=bd.scrollWidth-X+this.__num(bd,j)+this.__num(bd,f);}
;var bf=qx.bom.element.Style.get(bd,m);if(bf==b||bf==g){ba+=bd.scrollHeight-bc+this.__num(bd,i)+this.__num(bd,a);}
;}
;switch(be){case p:bh+=this.__num(bd,e);top+=this.__num(bd,r);Y-=this.__num(bd,x);ba-=this.__num(bd,q);case b:bh-=bd.scrollLeft;top-=bd.scrollTop;Y-=bd.scrollLeft;ba-=bd.scrollTop;case A:bh+=this.__num(bd,j);top+=this.__num(bd,i);Y-=this.__num(bd,f);ba-=this.__num(bd,a);break;case w:bh-=this.__num(bd,n);top-=this.__num(bd,l);Y+=this.__num(bd,s);ba+=this.__num(bd,k);break;};}
;return {left:bh,top:top,right:Y,bottom:ba};}
,__getBodyLocation:function(bj){var top=bj.offsetTop;var bk=bj.offsetLeft;top+=this.__num(bj,l);bk+=this.__num(bj,n);if(qx.core.Environment.get(u)===d){top+=this.__num(bj,j);bk+=this.__num(bj,i);}
...
,getOffsetParent:function(bu){if(bu instanceof SVGElement){return document.body;}
;var bt=bu.offsetParent||document.body;var bs=qx.bom.element.Style;while(bt&&(!/^body|html$/i.test(bt.tagName)&&bs.get(bt,v)===h)){bt=bt.offsetParent;}
;return bt;}
}});}
)();
...
;var v=function(){qx.module.Event.__isReady=true;u();}
...
else {var z=function(){if(qx.module.Event.__isReady){return;}
...
;var C=qx.module.Event.__normalizations;for(var i=0,l=E.length;i<l;i++ ){var D=E[i];if(qx.lang.Type.isFunction(B)){if(!C[D]){C[D]=[];}
;C[D].push(B);}
;}
;}
,$unregisterEventNormalization:function(I,F){if(!qx.lang.Type.isArray(I)){I=[I];}
;var G=qx.module.Event.__normalizations;for(var i=0,l=I.length;i<l;i++ ){var H=I[i];if(G[H]){qx.lang.Array.remove(G[H],F);}
;}
;}
...
;var M=qx.module.Event.__hooks.on;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(L)){if(!M[N]){M[N]=[];}
;M[N].push(L);}
;}
;if(!K){return;}
;var J=qx.module.Event.__hooks.off;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(K)){if(!J[N]){J[N]=[];}
;J[N].push(K);}
;}
;}
,$unregisterEventHook:function(U,R,Q){if(!qx.lang.Type.isArray(U)){U=[U];}
;var S=qx.module.Event.__hooks.on;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(S[T]){qx.lang.Array.remove(S[T],R);}
;}
;if(!Q){return;}
;var P=qx.module.Event.__hooks.off;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(P[T]){qx.lang.Array.remove(P[T],Q);}
;}
;}
,$getEventHookRegistry:function(){return qx.module.Event.__hooks;}
},members:{on:function(bd,bb,bc,W){for(var i=0;i<this.length;i++ ){var V=this[i];var Y=bc||qxWeb(V);var X=qx.module.Event.__hooks.on;var be=X[t]||[];if(X[bd]){be=be.concat(X[bd]);}
;for(var j=0,m=be.length;j<m;j++ ){be[j](V,bd,bb,bc);}
;var ba=function(bf,event){var bh=qx.module.Event.__normalizations;var bg=bh[t]||[];if(bh[bd]){bg=bg.concat(bh[bd]);}
...
;V.$$lastlistenerId=V.$$emitter.on(bd,ba,Y);V.$$emitter.getEntryById(V.$$lastlistenerId).useCapture=!!W;if(!V.__listener){V.__listener={};}
;if(!V.__listener[bd]){V.__listener[bd]={};}
;V.__listener[bd][V.$$lastlistenerId]=ba;if(!bc){if(!V.__ctx){V.__ctx={};}
;V.__ctx[V.$$lastlistenerId]=Y;}
;}
;return this;}
,off:function(bt,bo,bq,bj){var br=(bo===null&&bq===null);for(var j=0;j<this.length;j++ ){var bi=this[j];if(!bi.__listener){continue;}
;var bv=[];if(bt!==null){bv.push(bt);}
else {for(var bm in bi.__listener){bv.push(bm);}
;}
;for(var i=0,l=bv.length;i<l;i++ ){for(var bu in bi.__listener[bv[i]]){var bs=bi.__listener[bv[i]][bu];if(br||bs==bo||bs.original==bo){var bl=typeof bi.__ctx!==q&&bi.__ctx[bu];var bw;if(!bq&&bl){bw=bi.__ctx[bu];}
;var bp=bi.$$emitter.off(bv[i],bs,bw||bq);if(br||bs.original==bo){qx.bom.Event.removeNativeListener(bi,bv[i],bs,bj);}
;if(bp!==null){delete bi.__listener[bv[i]][bu];}
;if(bl){delete bi.__ctx[bu];}
;}
;}
;var bk=qx.module.Event.__hooks.off;var bn=bk[t]||[];if(bk[bt]){bn=bn.concat(bk[bt]);}
